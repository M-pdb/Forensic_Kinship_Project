...
# Extract lnLikelihood values from the FS_Linked summary file across all 50,000 iterations
# Save them as a single tab-delimited text file for further analysis

```{r}
# Define input summary file and output table path
in_summary  <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/FS_Linked/likelihoods_FS_Linked_summary.txt"
out_table   <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/FS_Linked/FS_Linked_lnlikelihoods.txt"
...
write_delim(FS, out_table, delim = "\t", col_names = TRUE)
...
# - FS_Linked vs FS_Linked_Unrelated (with linkage)
# - FS_Unlinked vs FS_Unlinked_Unrelated (no linkage)
FS_linked_fn         <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/FS_Linked/FS_Linked_lnlikelihoods.txt"
FS_linked_unrel_fn   <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/FS_Linked_Unrelated/FS_Linked_Unrelated_lnlikelihoods.txt"
FS_unlinked_fn       <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/FS_Unlinked/FS_Unlinked_lnlikelihoods.txt"
FS_unlinked_unrel_fn <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/FS_Unlinked_Unrelated/FS_Unlinked_Unrelated_lnlikelihoods.txt"
...
FS_linked <- read_tsv(
  FS_linked_fn,
  skip      = 1,
  col_names = c("replicate","lnL_FS_Linked"),
  col_types = cols(
    replicate       = col_character(),
    lnL_FS_Linked   = col_double()
  )
)

FS_linked_unrel <- read_tsv(
  FS_linked_unrel_fn,
  skip      = 1,
  col_names = c("replicate","lnL_FS_Linked_Unrelated"),
  col_types = cols(
    replicate                 = col_character(),
    lnL_FS_Linked_Unrelated   = col_double()
  )
)

FS_unlinked <- read_tsv(
  FS_unlinked_fn,
  skip      = 1,
  col_names = c("replicate","lnL_FS_Unlinked"),
  col_types = cols(
    replicate       = col_character(),
    lnL_FS_Unlinked = col_double()
  )
)

FS_unlinked_unrel <- read_tsv(
  FS_unlinked_unrel_fn,
  skip      = 1,
  col_names = c("replicate","lnL_FS_Unlinked_Unrelated"),
  col_types = cols(
    replicate                   = col_character(),
    lnL_FS_Unlinked_Unrelated   = col_double()
  )
)
...
LR_FS <- FS_linked %>%
  left_join(FS_linked_unrel,   by="replicate") %>%
  left_join(FS_unlinked,       by="replicate") %>%
  left_join(FS_unlinked_unrel, by="replicate")
...
LR_FS <- LR_FS %>%
  mutate(
    # natural‐log LRs
    logLR_linked   = lnL_FS_Linked   - lnL_FS_Linked_Unrelated,
    logLR_unlinked = lnL_FS_Unlinked  - lnL_FS_Unlinked_Unrelated,
...
summary_FS <- tibble(
  ...
  median_logLR = c(
    median(LR_FS$log10LR_linked,   na.rm=TRUE),
    median(LR_FS$log10LR_unlinked, na.rm=TRUE)
  ),
  min_logLR    = c(
    min(LR_FS$log10LR_linked,   na.rm=TRUE),
    min(LR_FS$log10LR_unlinked, na.rm=TRUE)
  ),
  max_logLR    = c(
    max(LR_FS$log10LR_linked,   na.rm=TRUE),
    max(LR_FS$log10LR_unlinked, na.rm=TRUE)
  )
)

# Save the full LR results and summary statistics to .txt files
write_tsv(LR_FS,       "FS_all_LR_results.txt")
write_tsv(summary_FS,   "FS_LR_summary_stats.txt")
...
UN_linked_fn         <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/UN_Linked/UN_Linked_lnlikelihoods.txt"
UN_linked_unrel_fn   <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/UN_Linked_Unrelated/UN_Linked_Unrelated_lnlikelihoods.txt"
UN_unlinked_fn       <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/UN_Unlinked/UN_Unlinked_lnlikelihoods.txt"
UN_unlinked_unrel_fn <- "/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/UN_Unlinked_Unrelated/UN_Unlinked_Unrelated_lnlikelihoods.txt"
