...
# Visualise and compare likelihood ratios (LRs) for FS and UN scenarios using density plots on a log10 scale

```{r}
# Load LR results from CSV files for FS and UN data
FS <- read.csv("/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/Outputs/FS_all_LR_results.csv")
un <- read.csv("/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FS_Simulation/Outputs/UN_all_LR_results.csv")
...
FS_tidy <- FS %>% 
  ...
  mutate(
    ...
    Group = recode(
      Scenario,
      log10LR_linked   = "TrueFS_Linked",
      log10LR_unlinked = "TrueFS_Unlinked"
    )
  )
...
plot_data <- bind_rows(FS_tidy, un_tidy)
...
p <- ggplot(plot_data, aes(x = LR, colour = Group, fill = Group)) +
  ...
  labs(
    title = "Distribution of LRs for Full Siblings vs Unrelated with 94 SNPs",
    x     = "LRs (log scale)",
    y     = "Density"
  ) +
  theme_minimal() +
