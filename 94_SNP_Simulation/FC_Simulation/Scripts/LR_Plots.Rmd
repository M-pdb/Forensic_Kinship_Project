...
# Visualise and compare likelihood ratios (LRs) for FC and UN scenarios using density plots on a log10 scale

```{r}
# Load LR results from CSV files for FC and UN data
FC <- read.csv("/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FC_Simulation/Outputs/FC_all_LR_results.csv")
un <- read.csv("/Users/huncho/Desktop/Forensic_Kinship/94_SNP_Simulation/FC_Simulation/Outputs/UN_all_LR_results.csv")
...
FC_tidy <- FC %>% 
  ...
  mutate(
    ...
    Group = recode(
      Scenario,
      log10LR_linked   = "TrueFC_Linked",
      log10LR_unlinked = "TrueFC_Unlinked"
    )
  )
...
plot_data <- bind_rows(FC_tidy, un_tidy)
