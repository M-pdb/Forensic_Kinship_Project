...
in_dir  <- "/scratch/users/k21040223/HS_Simulation/UN_Linked"
out_dir <- "/scratch/users/k21040223/HS_Simulation/UN_Linked_Unrelated"
...
process_ped <- function(in_ped, out_ped) {
  ...
  df_mod <- df %>%
    ...
    # Shift genotypes from rows 4 and 5 to become the first two rows (representing two unrelated individuals)
    {
      tmp  <- .
      cols <- 6:ncol(tmp)
      tmp[1, cols] <- tmp[4, cols]
      tmp[2, cols] <- tmp[5, cols]
      tmp
    } %>%
...
```{r}
# Define file paths
Simulated_UN_ped1_modified_UN_path <- "/Users/huncho/Desktop/Forensic_Kinship/10,000_SNP_Simulation/HS_Simulation/UN_Linked_Unrelated/UN_ped_00001-Unrelated.ped"
